...
def load_gptq_model(
    ...
    model = AutoGPTQForCausalLM.from_quantized(
        ...
        trust_remote_code=trust_remote_code,
        device="cuda:0",
        use_triton=inject_fused_attention,
...
def load_from_catalogue(model_name):
    ...
    catalogue = {
        ...
        "TheBloke/guanaco-65B-GPTQ": {
            "load_method": lambda x: load_gptq_model(
                x,
                trust_remote_code=True,  # might not need the trust remote code
            ),
            "user_prompt": "### Human:",
        ...
        "psmathur/orca_mini_v3_7b": {
            "load_method": lambda x: load_automodel(
                x, trust_remote_code=True, load_in_4bit=True
            ),
    ...
    return (
        model,
        tokenizer,
        catalogue[model_name]["user_prompt"],
        catalogue[model_name]["assistant_prompt"],
    )
